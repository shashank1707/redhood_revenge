...
class NightBorne extends SpriteAnimationComponent
  ...
  void setTimers() {
    attackTimer = Timer(0.9, callback: () {
      if (this.stillColliding) {
        gameRef.warriorComponent.health.value -= this.enemyDamage;
        gameRef.warriorComponent.animation =
            gameRef.warriorComponent.hurtAnimation;
        gameRef.warriorComponent.isHurting = true;
        gameRef.warriorComponent.hurtTimer.start();
      }
  ...
  void findHero() {
    if (!this.isHurting) {
      if ((this.distance(gameRef.warriorComponent) < 200 &&
          this.distance(gameRef.warriorComponent) > this.hitBox.x)) {
        moveTodardsHero();
      } else {
        stopMoving();
      }
  ...
  void moveTodardsHero() {
    if (this.x > gameRef.warriorComponent.x + this.hitBox.x / 2) {
      this.renderFlipX = true;
      this.enemyCurrentSpeed = -1 * this.enemySpeed;
      if (!this.isAttacking && !this.isHurting && !this.isDying)
        this.animation = runAnimation;
    } else if (this.x < gameRef.warriorComponent.x - this.hitBox.x / 2) {
  ...
  void attack() async {
    if (!this.isAttacking &&
        gameRef.warriorComponent.y >= this.y - this.hitBox.y / 2) {
  ...
  void checkIfHit() async {
    if (gameRef.warriorComponent.hasAttacked &&
        gameRef.warriorComponent.y >= this.y - this.hitBox.y / 2 &&
        this.stillColliding &&
        !this.isHurting) {
      gameRef.warriorComponent.hasAttacked = false;
      this.enemyHealth.value -= this.enemyHPRate;
      this.animation = hurtAnimation;
      this.isHurting = true;
      this.hurtTimer.start();

      if (gameRef.warriorComponent.x < this.x) {
